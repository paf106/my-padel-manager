...
export async function createSeries(formData: FormData) {
  ...
  const expected =
    ...
    (result.data.type === "individual"
      ? studentIds.length === 1
      : result.data.type === "pair"
        ? studentIds.length === 2
        : [3, 4].includes(studentIds.length));
  ...
  if (validStudents.length !== new Set(studentIds).size)
    redirect("/classes/series/new?error=invalid");
  const dateKeys = madridSeriesDateKeys(
    result.data.startsOn,
    result.data.weekday,
    result.data.weeks,
  );
  await db.transaction(async (tx) => {
    const [series] = await tx
      ...
      .values({
        ...
        courtPriceCents: result.data.courtPriceCents,
        ratePerStudentCents: result.data.ratePerStudentCents,
    ...
    const created = await tx
      ...
      .values(
        dateKeys.map((dateKey) => ({
          seriesId: series.id,
          type: result.data.type,
          startsAt: madridFromLocalInput(`${dateKey}T${result.data.timeOfDay}`),
          durationMin: result.data.durationMin,
          courtPriceCents: result.data.courtPriceCents,
          ratePerStudentCents: result.data.ratePerStudentCents,
        })),
    ...
    await tx
      ...
      .values(
        created.flatMap(({ id }) => studentIds.map((studentId) => ({ classId: id, studentId }))),
      );
  });
  revalidatePath("/");
