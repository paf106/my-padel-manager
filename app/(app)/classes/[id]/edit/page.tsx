...
export default async function EditClassPage({
  ...
  const [[item], selectedRows, paymentRows] = await Promise.all([
    ...
    db
      .select({ paymentId: paymentLines.paymentId })
      .from(paymentLines)
      .where(eq(paymentLines.classId, id))
      .limit(1),
  ]);
  if (!item)
    return (
      <main className="mx-auto max-w-3xl">
        <PageHeader title="Clase no encontrada" eyebrow="Clases" backHref="/calendar" />
      </main>
    );
  ...
  const studentRows = await db
    ...
    .from(students)
    .where(or(eq(students.active, true), inArray(students.id, [...selected])));
  const localDateTime = madridLocalInputValue(item.startsAt);
  return (
    <main className="mx-auto max-w-3xl">
      <PageHeader title="Editar clase" eyebrow="Clases" backHref={`/classes/${id}`} />
      {error && (
        <p className="mt-4 rounded-xl bg-red-50 px-3 py-2 text-sm font-bold text-red-700">
          El tipo debe coincidir con el número de alumnos.
        </p>
      )}
      {paymentRows.length > 0 && (
        <p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">
          Esta clase ya está incluida en un pago generado. Si cambias su estado o sus precios,
          vuelve a pulsar &quot;Generar pagos&quot; en el mes correspondiente.
        </p>
      )}
      <form
      ...
      >
        ...
        <Field label="Fecha y hora">
          <Input name="startsAt" type="datetime-local" required defaultValue={localDateTime} />
        </Field>
        <StudentPicker students={studentRows} selectedIds={[...selected]} />
        <Field label="Precio pista">
          <MoneyInput
            name="courtPriceCents"
            defaultValue={(item.courtPriceCents / 100).toFixed(2)}
          />
        </Field>
        <Field label="Precio clase / alumno">
          <MoneyInput
            name="ratePerStudentCents"
            defaultValue={(item.ratePerStudentCents / 100).toFixed(2)}
          />
        </Field>
        <Field label="Duración">
          <Input name="durationMin" type="number" min="1" defaultValue={item.durationMin} />
        </Field>
