...
export default async function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [[item], classmates] = await Promise.all([db.select().from(classes).where(eq(classes.id, id)), db.select({ student: students }).from(classStudents).innerJoin(students, eq(classStudents.studentId, students.id)).where(eq(classStudents.classId, id))]);
