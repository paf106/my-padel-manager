...
export default async function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl pb-12"><div className="flex items-start justify-between"><BackLink href="/calendar" label="Volver al calendario" /><Link href={`/classes/${id}/edit`} className="rounded-xl bg-emerald-800 px-4 py-3 text-sm font-bold text-white">Editar</Link></div><h1 className="mt-6 text-3xl font-black">{format(item.startsAt, "d/MM · HH:mm")}</h1><div className="mt-2 flex items-center gap-3"><p className="text-slate-500">{classTypeLabels[item.type]} · {item.durationMin} min</p><Badge tone={item.status === "completed" ? "success" : item.status === "cancelled" ? "danger" : "warning"}>{classStatusLabels[item.status]}</Badge></div><div className="mt-8 grid gap-6 lg:grid-cols-2"><section className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm font-bold text-slate-500">Resumen económico</p><dl className="mt-3 divide-y divide-slate-100"><Info label="Precio de clase" value={formatMoney(item.ratePerStudentCents)} /><Info label="Precio de pista" value={formatMoney(item.courtPriceCents)} /></dl><h2 className="mt-6 text-sm font-bold text-slate-500">Alumnos</h2><div className="mt-3 space-y-2">{classmates.map(({ student }) => <p key={student.id} className="font-bold">{student.firstName} {student.lastName}</p>)}</div></section><div><section className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm font-bold text-slate-500">Comentario</p><p className="mt-3 whitespace-pre-wrap text-sm">{item.notes || <span className="text-slate-400">Sin comentario</span>}</p></section><form action={deleteClass.bind(null, id)} className="mt-5"><button className="text-sm font-bold text-red-700">Eliminar clase</button></form></div></div></main>;
}
