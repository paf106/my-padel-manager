...
export async function updateClass(id: string, formData: FormData) {
  ...
  if (
    ...
    !validateClassStudentCount(result.data.type, studentIds.length)
  )
    redirect(`/classes/${id}/edit?error=invalid`);
  ...
  await db.transaction(async (tx) => {
    await tx
      .update(classes)
      .set({ ...result.data, startsAt, notes: result.data.notes || null, updatedAt: new Date() })
      .where(eq(classes.id, id));
    await tx.delete(classStudents).where(eq(classStudents.classId, id));
    await tx
      .insert(classStudents)
      .values(studentIds.map((studentId) => ({ classId: id, studentId })));
  });
  revalidatePath("/");
  revalidatePath("/calendar");
