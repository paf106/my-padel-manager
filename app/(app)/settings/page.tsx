  ...
  return (
    <main className="mx-auto max-w-3xl">
      ...
      <p className="mt-2 text-slate-500">
        Estos precios se usarán como valor inicial en las nuevas clases.
      </p>
      <form
        action={updateSettings}
        className="mt-8 grid gap-5 rounded-2xl border border-slate-200 bg-white p-5 lg:grid-cols-2"
      >
        <Field label="Precio habitual de pista">
          <MoneyInput
            name="defaultCourtPriceCents"
            defaultValue={(current.defaultCourtPriceCents / 100).toFixed(2)}
          />
        </Field>
        <Field label="Clase individual">
          <MoneyInput
            name="rateIndividualCents"
            defaultValue={(current.rateIndividualCents / 100).toFixed(2)}
          />
        </Field>
        <Field label="Clase pareja · por alumno">
          <MoneyInput
            name="ratePairCents"
            defaultValue={(current.ratePairCents / 100).toFixed(2)}
          />
        </Field>
        <Field label="Clase grupo · por alumno">
          <MoneyInput
            name="rateGroupCents"
            defaultValue={(current.rateGroupCents / 100).toFixed(2)}
          />
        </Field>
        <Field label="Duración por defecto (minutos)">
          <Input
            ...
            min="1"
            step="1"
            defaultValue={current.defaultDurationMin}
          />
        </Field>
