...
export default async function DashboardPage() {
  ...
  const chartData = Array.from({ length: 6 }, (_, index) => { const date = new Date(now.getFullYear(), now.getMonth() - 5 + index, 1); const period = date.toISOString().slice(0, 10); return { label: format(date, "MMM"), amount: recentPayments.filter((payment) => payment.period === period).reduce((total, payment) => total + payment.amount, 0) / 100 }; });
