...
export async function generateMonthlyPayments(period: string) {
  ...
  const billingClasses = [...grouped.values()].map((rows) => ({ id: rows[0].padelClass.id, type: rows[0].padelClass.type, courtPriceCents: rows[0].padelClass.courtPriceCents, ratePerStudentCents: rows[0].padelClass.ratePerStudentCents, studentIds: rows.map((row) => row.studentId).sort() }));
  ...
  await db.transaction(async (tx) => {
    const existing = await tx.select({ id: payments.id, studentId: payments.studentId, paidAmountCents: payments.paidAmountCents }).from(payments).where(eq(payments.period, `${period}-01`));
    const pendingStudentIds = [...totals.keys()].filter((studentId) => !(existing.find((payment) => payment.studentId === studentId)?.paidAmountCents ?? 0));
    const pendingRows = pendingStudentIds.map((studentId) => ({ studentId, period: `${period}-01`, computedAmountCents: totals.get(studentId)! }));
    const updatedPayments = pendingRows.length === 0 ? [] : await tx.insert(payments).values(pendingRows).onConflictDoUpdate({ target: [payments.studentId, payments.period], set: { computedAmountCents: sql`excluded.computed_amount_cents`, updatedAt: new Date() } }).returning({ id: payments.id, studentId: payments.studentId });
    ...
    if (paymentIds.length > 0) {
      ...
      const nextLines = updatedPayments.flatMap((payment) => lines.filter((line) => line.studentId === payment.studentId).map((line) => ({ paymentId: payment.id, classId: line.classId, studentCount: grouped.get(line.classId)?.length ?? 0, classShareCents: line.classShareCents, courtShareCents: line.courtShareCents, amountCents: line.amountCents })));
