...
import { generateMonthlyPayments, deletePayment } from "./actions";
...
export default async function PaymentsPage({
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl pb-28 lg:pb-4">
      <PageHeader title="Pagos" eyebrow="Finanzas" />
      <div className="mt-6 rounded-2xl bg-emerald-900 p-5 text-white">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <form action={generateMonthlyPayments.bind(null, period)}>
              <button className="min-h-11 rounded-xl bg-white px-4 text-sm font-bold text-emerald-900">
                Generar pagos
              </button>
            </form>
          </div>
        ...
        <>
          <div className="mt-6 space-y-3 lg:hidden">
            {rows.map(({ payment, student }) => (
              <PaymentCard key={payment.id} payment={payment} student={student} />
            ))}
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th className="px-5 py-4">Cobrado</th>
                ...
                {rows.map(({ payment, student }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">{formatMoney(due)}</td>
                      <td className="px-5 py-4">{formatMoney(payment.paidAmountCents)}</td>
                      <td className="px-5 py-4">
                        {formatMoney(Math.max(0, due - payment.paidAmountCents))}
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <ConfirmButton
                          ...
                          className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-red-700 hover:bg-red-50"
                        >
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <Link href={`/payments/${payment.id}`} className="block">
        <div className="flex items-center justify-between">
          ...
          <Badge
            tone={status === "completed" ? "success" : status === "partial" ? "warning" : "danger"}
          >
            {paymentStatusLabels[status]}
          </Badge>
        </div>
