...
export function WeekStrip({
  ...
  for (const day of days)
    daysByWeek.set(day.weekStart, [...(daysByWeek.get(day.weekStart) ?? []), day]);
        ...
        {weeks.map((weekStart) => (
          <div key={weekStart} className="grid min-w-full snap-center grid-cols-7 gap-1">
            {daysByWeek.get(weekStart)?.map((day) => (
              <Link
                key={day.date}
                href={buildCalendarHref("/calendar", year, month, day.date)}
                className="flex min-h-20 flex-col items-center justify-center gap-1"
              >
                <span
                  className={`text-sm capitalize ${day.date === selectedDay ? "text-emerald-700" : "text-slate-700"}`}
                >
                  {day.weekdayLabel}
                </span>
                <span
                  className={`flex h-11 w-11 items-center justify-center rounded-full text-lg font-bold ${day.date === selectedDay ? "bg-emerald-800 text-white" : day.isToday ? "ring-2 ring-emerald-800" : ""}`}
                >
                  {day.dayNumber}
                </span>
                <span
                  className={`h-1.5 w-1.5 rounded-full ${classDays.has(day.date) ? "bg-lime-500" : "bg-transparent"}`}
                />
              </Link>
            ))}
          </div>
