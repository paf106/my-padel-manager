...
export function NavLink({ href, label, icon, variant }: { href: string; label: string; icon: keyof typeof icons; variant: "sidebar" | "bottom" }) {
  ...
  if (variant === "bottom") {
    return <Link href={href} aria-current={active ? "page" : undefined} className={cn("flex min-h-11 flex-1 flex-col items-center gap-1 text-xs font-bold", active ? "text-primary" : "text-slate-500")}><span className={cn("flex h-7 w-10 items-center justify-center rounded-lg", active && "bg-primary text-white")}><Icon size={21} /></span><span>{label}</span></Link>;
  }
  return <Link href={href} aria-current={active ? "page" : undefined} className={cn("flex min-h-12 items-center gap-3 rounded-xl px-3 text-sm font-bold transition-colors", active ? "bg-primary text-white" : "text-slate-600 hover:bg-slate-100 hover:text-primary")}><Icon size={20} /><span>{label}</span></Link>;
}
